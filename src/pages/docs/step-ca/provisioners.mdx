...
- `disableRenewal`: disables renewal of certificates using [`step ca renew`](/docs/step-cli/reference/ca/renew) or [`step ssh renew`](/docs/step-cli/reference/ssh/renew) (default `false`)

- `allowRenewAfterExpiry`: allows renewal of expired certificates using [`step ca renew`](/docs/step-cli/reference/ca/renew) or [`step ssh renew`](/docs/step-cli/reference/ssh/renew) (default `false`)
...
- `enableSSHCA`: enable this provisioner to generate SSH Certificates. (default `false`).
...
Because `step ca init` creates an ECDSA chain by default, you will need to [convert your CA to use an RSA CA chain](/docs/tutorials/rsa-chain) before using the SCEP provisioner.
