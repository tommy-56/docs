...
This table shows some of the feature differences between an online `step-ca` server, `step` CLI in offline mode, and the [`step certificate` subcommand](/docs/step-cli/reference/certificate).

![](/graphics/cas-three-ways.png)

### Example: Offline Mode

Let's create a certificate without running `step-ca`.
First, initialize a `step-ca` PKI:

```shell-session
step ca init --name "Local CA" --provisioner admin --dns localhost --address ":443"
```

Now, without starting `step-ca`, you can get a certificate using offline mode:

```shell-session
step ca certificate --offline foo.smallstep.com foo.crt foo.key
```

### Example: `step certificate`

You can also use the [`step certificate` subcommand](/docs/step-cli/reference/certificate) to create CAs and leaf certificates offline, without initializing a PKI or running a `step-ca` server.

For example, you can use [`step certificate`](/docs/step-cli/reference/certificate) to create a self-signed leaf certificate:
...
See the [Basic Crypto Operations](/docs/step-cli/basic-crypto-operations) section of the `step` CLI documentation for more examples.
