...
If you need to place a load balancer or reverse proxy downstream from the CA,
we recommend using layer 4 (TCP) load balancing or proxying (aka "TLS passthrough").

Layer 7 proxying is _not recommended_, because the `step` toolchain is built around TLS:
...
Finally, when load balancing, your `step-ca` instances should share a single MySQL or PostgreSQL database.
...
or pass the `--not-after` flag to the [`step ca certificate`](/docs/step-cli/reference/ca/certificate) to adjust the
...
Here are three options for setting up automated renewal of certificates using [`step ca renew`](/docs/step-cli/reference/ca/renew):
...
With this method, [`step ca renew`](/docs/step-cli/reference/ca/renew) operates as a daemon that will keep your certificates up-to-date:
...
When daemonized, [`step ca renew`](/docs/step-cli/reference/ca/renew) will attempt a renewal when the certificate's lifetime is approximately two-thirds elapsed.
...
You can trigger renewal anytime by sending a `SIGHUP` signal to the [`step ca renew`](/docs/step-cli/reference/ca/renew) process ID.

You can add [`step ca renew --daemon`](/docs/step-cli/reference/ca/renew) as a `systemd` service that runs on startup and restarts as needed.
...
To address this, [`step ca renew`](/docs/step-cli/reference/ca/renew) can send a `SIGHUP` to your service after each renewal.
...
With `cron`-based renewal, you can have [`step ca renew`](/docs/step-cli/reference/ca/renew) run at a regular cadence (eg. every five minutes) and renew a certificate only when it's approaching its expiration date.
...
[`step ca renew`](/docs/step-cli/reference/ca/renew) allows a certificate owner to extend the lifetime of a certificate before it expires.
