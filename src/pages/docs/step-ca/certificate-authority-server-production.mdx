...
- [Running `step-ca` on a Public IP](#running-step-ca-on-a-public-ip)
...
## Running `step-ca` on a Public IP

You can run `step-ca` on a public IP, but you need to add some additional protections to your configuration:

- If you're using an OIDC provisioner,
add the `domains` configuration property to limit the token email domains that will be accepted by the CA.
Do not link the OIDC provisioner to a public email domain like `gmail.com`.
- If you're using any cloud provisioners,
restrict the accounts that can use IIDs to get certificates:
  - For GCP, use the `serviceAccounts` and `projectIDs` properties.
  - For AWS, use the `accountId` property.
  - For Azure, use the `tenantId` and `resourceGroups` properties.
- To allow clients to bootstrap, `step-ca` makes some information public:
  - The provisioner configurations (at `/provisioners`)
  - Your root certificate chain
  - The health check endpoint (`/health`).
- If you're using the JWK provisioner, the encrypted key is public, so it is critical that you use a strong password for it. Optionally, you can remove the JWK `encryptedKey` from the CA's publicly-accessible provisioner configuration. While the `encryptedKey` improves the client UX, is not required for CA operation. If removed, clients will need to pass in the key via `step ca token --key=jwk.key`

As an alternative to publicly hosting `step-ca`, you can tunnel `step-ca` traffic using TLS-over-mutual TLS.
This is a more secure configuration, but with security comes complexity.
This is an experimental feature that uses [`ghostunnel`](https://github.com/ghostunnel/ghostunnel).
See [certificates#531](https://github.com/smallstep/certificates/pull/531) for details.
